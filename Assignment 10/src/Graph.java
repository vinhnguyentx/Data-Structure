...
public class Graph {
	...
	// dijkstra code is from the lecture
	public int dijkstra( Vertex s ) {
		...
		int count = 0; //count of the number of nodes removed
			...
			if ( ! v.visited ) {
				...
				for ( Edge e : v.edges ) {
					...
					if ( newcost < e.target.cost ) {
						...
						fringe.add(e.target); // update fringe
					}
				}
			}
	...
	public static final Heuristic randomlies = new Heuristic() {
		public Integer fn(Vertex from, Vertex to) {
			return (Integer) ( (int) (rng.nextDouble() * 5000) ); 
		}
	};
	...
	// create a list of city names to get to specified city
	public Cons pathto(String city) {
		Cons citieslist = Cons.list(city);
		while ( vertex(city).parent() != null ) {
			citieslist = Cons.append( Cons.list( vertex(city).parent().contents ), citieslist );
